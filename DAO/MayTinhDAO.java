...
public class MayTinhDAO {
    ...
    // Thêm máy tính mới
    public void addPC(MayTinhEntity mt) {
        String sql = "INSERT INTO May_tinh(ten_may, id_khu_vuc) VALUES (?, ?)";
        try (Connection conn = KetNoiDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, mt.getTen_may());
            ps.setInt(2, mt.getId_khu_vuc());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void updatePCInfo(int id, String tenMay, int idKhuVuc) {
    ...
    while (rs.next()) {
        ...
        pc.setTenMay(rs.getString("ten_may"));
